...
describe("PlatformSpawnSystem", () => {
	...
	beforeEach(async () => {
		...
		world = h.world;
		system = new PlatformSpawnSystem(
			mockRandom,
			mockRandomSize,
			mockRandomRocket,
		);
	...
	describe("makeWeightedPicker", () => {
		it("корректно выбирает значения с учётом весов", () => {
			...
			const testSystem = new PlatformSpawnSystem(
				testRandom,
				testRandomSize,
				testRandomRocket,
			);
	...
	describe("spawnRandomPlatform", () => {
		it("спавнит платформу в случайной позиции внутри AABB", () => {
			...
			expect(result.position[0] - halfWidth).toBeGreaterThanOrEqual(
				aabb.min[0],
			);
			expect(result.position[0] + halfWidth).toBeLessThanOrEqual(aabb.max[0]);
			expect(result.position[1] - halfHeight).toBeGreaterThanOrEqual(
				aabb.min[1],
			);
	...
	describe("spawnDefaultSet", () => {
		it("спавнит 5 платформ", () => {
			mockRandom.reset([
				0.5, 0.5, 0.2, 0.5, 0.5, 0.2, 0.5, 0.5, 0.2, 0.5, 0.5, 0.2, 0.5, 0.5,
				0.2,
			]);
		...
		it("спавнит платформы выше стартовой позиции по Y", () => {
			...
			mockRandom.reset([
				0.5, 0.3, 0.2, 0.5, 0.5, 0.2, 0.5, 0.7, 0.2, 0.5, 0.6, 0.2, 0.5, 0.8,
				0.2,
			]);
			mockRandomSize.reset([0.5, 0.5, 0.5, 0.5, 0.5]);
	...
	describe("update", () => {
		...
		it("не спавнит платформы если игрок не пересек trigger range", () => {
			...
			const initialPlatformCount = Array.from(
				world.query(TransformComponent, ColliderComponent),
			).length;

			system.fixedUpdate(world);

			const finalPlatformCount = Array.from(
				world.query(TransformComponent, ColliderComponent),
			).length;
		...
		it("не спавнит платформы если игрок ниже trigger range", () => {
			...
			spawner.lastPlatform = world.addEntity(
				createPlatform({
					position: Vec2.fromValues(500, 0),
					kind: "default",
				}),
