...
export class DefenseController {
	...
	selectCell(cell: number): void {
		...
		if (selection.mode === "build" && !occupied) {
			this.#message = this.session.dispatch({
				type: "tower.build",
				cell,
				kind: selection.kind,
			})
	...
	act(id: ConstructionAction): void {
		...
		switch (id) {
			...
			case "overdrive":
				if (cell !== null)
					this.session.dispatch({ type: "tower.overdrive", cell });
				break;
			case "priority": {
				const tower = this.session
					.snapshot()
					.towers.find((t) => t.slot === cell);
				const priorities = ["first", "shield", "strong", "support"] as const;
				if (tower)
					this.session.dispatch({
						type: "tower.priority",
						cell: tower.slot,
						priority:
							priorities[
								(priorities.indexOf(tower.priority) + 1) % priorities.length
							],
					});
				break;
			}
			...
			case "improve":
				if (cell !== null)
					this.session.dispatch({ type: "tower.improve", cell });
				break;
			case "sell":
				if (
					cell !== null &&
					this.session.dispatch({ type: "tower.sell", cell })
				)
					this.#selection = inspect();
				break;
			case "move":
			...
			case "replace":
				...
				break;
			case "startWave":
				if (this.session.dispatch({ type: "wave.start" }))
					this.#selection = inspect();
				break;
			case "portalBack":
			...
			case "placeWorld":
				...
				if (
					selection.mode === "snow" &&
					this.session.dispatch({ type: "world.placeSnow", cell })
				)
					this.#selection = inspect();
