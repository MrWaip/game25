...
function forceGC(): void {
	if (
		...
		typeof (globalThis as unknown as { global?: { gc?: () => void } }).global
			?.gc === "function"
	) {
...
describe("Стресс-тест производительности", () => {
	it("должен пройти 10 секунд реального времени и собрать метрики FPS и памяти", async () => {
		...
		console.log(
			`Запуск стресс-теста на ${testDurationSeconds} секунд реального времени...`,
		);
