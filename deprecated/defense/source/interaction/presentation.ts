...
export function present(
	...
	if (view.mode === "build" && view.kind) {
		...
		if (tower.level >= 2 && !tower.specialization) {
			const focus =
				tower.kind === "amplifier"
					? "+35% ускорения"
					: tower.kind === "corrode"
						? "Вдвое быстрее снять броню"
						: tower.kind === "frost"
							? "+2 с замедления"
							: "×1.8 урона";
			const spread =
					...
					: tower.kind === "rapid"
						...
						: tower.kind === "arcane"
							...
							: tower.kind === "corrode"
								? "+1 цель"
								: "Больше площадь";
			add(`Фокус: ${focus} · 35`, "focus", state.coins < 35);
			add(`Охват: ${spread} · 35`, "spread", state.coins < 35);
		}
	}

	if (tower && state.phase === "wave") {
		actions.length = 0;
		const labels = {
			first: "Ближе к базе",
			shield: "Щиты",
	...
	return {
		...
		hint:
			...
			(state.phase === "wave"
				? tower
					? hint
					: "Выбери башню: Форсаж или приоритет цели · 2 приказа за волну"
				: hint),
		actions: prepare || state.phase === "wave" ? actions : [],
		choices,
		launch: prepare
			? {
					id: "startWave",
					label: state.expansionDue
						? "Выбери расширение ниже"
						: state.pendingWorld
							? "Сначала размести эффект мира"
							: `Начать волну ${state.wave + 1}`,
					disabled: state.pendingWorld !== null || state.expansionDue,
					tone: "primary",
				}
			: null,
