...
type ConstructionState = Pick<
	DefenseSnapshot,
	"phase" | "coins" | "towers" | "map"
>;
const longestRange = Math.max(
	...Object.values(towerDefinitions).map((tower) => tower.range),
);
/** A land cell is a useful construction site only if a weapon can reach a route. */
export function isConstructionSite(
	...
	return (
		isBuildable(map, cell) &&
		distanceToRoad(map, cell) <
			(kind ? towerDefinitions[kind].range : longestRange)
	);
}
