...
export class PlatformSpawnSystem implements ISystem {
  ...
  spawnBonusRocketPlatform(
    ...
    const leftAvailableMin =
      ctx.spawner.minX + 10 + bonusPlatformHalfWidth;
    const leftAvailableMax =
      lastPlatformLeftCornerX - bonusPlatformHalfWidth;
    const leftBoundaryMin = Math.max(
      leftAvailableMin,
      lastPlatformLeftCornerX - MAX_JUMP_WIDTH - bonusPlatformHalfWidth
    );
    const leftAvailableDistance = Math.max(0, leftAvailableMax - leftBoundaryMin);

    const rightAvailableMin = lastPlatformRightCornerX + bonusPlatformHalfWidth;
    const rightAvailableMax =
      ctx.spawner.maxX - 10 - bonusPlatformHalfWidth;
    const rightBoundaryMax = Math.min(
      rightAvailableMax,
      lastPlatformRightCornerX + MAX_JUMP_WIDTH + bonusPlatformHalfWidth
    );
    const rightAvailableDistance = Math.max(0, rightBoundaryMax - rightAvailableMin);
    ...
    const trySpawnOnSide = (
      ...
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        ...
        for (const platform of spawnedPlatforms) {
          const platformAABB = AABB.fromCenter(platform.position, platform.size);
    ...
    const bonusEntity = world.addEntity(
      createPlatform({ position: bonusPosition, size: bonusSize, kind: "default" })
    );
