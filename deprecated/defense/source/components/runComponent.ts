...
export class Run extends Component {
	...
	overdrives = 2;
	harvestKills = 0;
	/** Transient feedback; gameplay counters are saved separately. */
	triggers: Partial<Record<Upgrade, number>> = {};
	towerLimit = expansion.initialLimit;
	sites: number[];
	openSites = 0;
	expansionDue = false;
	shop: Upgrade[] = [];
	shopRoll = 0;
	heldOffer: Upgrade | null = null;
	constructor(
		public seed: string,
		public map: DefenseMap = generateMap(seed),
	) {
		super();
		this.sites = foundations(map);
		this.openSites = this.sites.length;
	}
}
