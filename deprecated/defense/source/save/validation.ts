import { foundations, expansion } from "@/games/defense/expansion";
...
function validState(value: unknown): value is SavedRun {
	...
	if (
		...
		return false;
	if (
		!integer(value.towerLimit) ||
		value.towerLimit < 1 ||
		value.towerLimit > value.map.columns * value.map.rows ||
		!Array.isArray(value.sites) ||
		!value.sites.every(
			(site) =>
				integer(site) && isBuildable(value.map as SavedRun["map"], site),
		) ||
		new Set(value.sites).size !== value.sites.length ||
		!integer(value.openSites) ||
		value.openSites > value.sites.length ||
		typeof value.expansionDue !== "boolean" ||
		!integer(value.shopRoll) ||
		!Array.isArray(value.shop) ||
		value.shop.length > 3 ||
		new Set(value.shop).size !== value.shop.length ||
		!value.shop.every(
			(key) => typeof key === "string" && Object.hasOwn(upgrades, key),
		) ||
		(value.heldOffer !== null &&
			(typeof value.heldOffer !== "string" ||
				!value.shop.includes(value.heldOffer)))
	)
		return false;
	if (
		...
		return false;
	if (value.phase !== "reward" && value.choices.length !== 0) return false;
	if (
	...
	if (
		...
		!value.towers.every(
			(t) =>
				...
				number(t.overdrive) &&
				t.overdrive <= 4 &&
				["first", "shield", "strong", "support"].includes(String(t.priority)) &&
				(t.specialization === null ||
					t.specialization === "focus" ||
					t.specialization === "spread"),
		)
	)
		return false;
	...
	return (
		...
		value.enemies.every(
			(e) =>
				...
				number(e.corrosion) &&
				e.corrosion <= 0.65 &&
				number(e.corrosionTime) &&
				integer(e.charge) &&
				e.charge < 5 &&
				(e.lastDamage === null ||
					e.lastDamage === "physical" ||
					e.lastDamage === "magic"),
		)
	);
}
export function decodeSave(serialized: string): SavedRun {
	...
	if (
		!record(value) ||
		![2, 3, 4, 5, 6, saveVersion].includes(Number(value.version)) ||
	...
	if (Number(value.version) < 7 && record(state)) {
		state.overdrives = 2;
		state.harvestKills = 0;
		if (validMap(state.map)) {
			const occupied = Array.isArray(state.towers)
				? state.towers.flatMap((t) =>
						record(t) && integer(t.slot) ? [t.slot] : [],
					)
				: [];
			state.sites = [...new Set([...occupied, ...foundations(state.map)])];
			state.openSites = (state.sites as number[]).length;
			state.towerLimit = Math.max(expansion.initialLimit, occupied.length);
		}
		state.expansionDue = false;
		state.shop = [];
		state.shopRoll = 0;
		state.heldOffer = null;
		if (record(state.bonuses))
			for (const key of [
				"charge",
				"acidBurst",
				"execution",
		...
		if (Array.isArray(state.towers))
			state.towers = state.towers.map((t) =>
				record(t)
					? { ...t, priority: "first", specialization: null, overdrive: 0 }
					: t,
			);
		if (Array.isArray(state.enemies))
