...
import { FakeInputStrategy } from "@/games/jumper/testkit/fakeInputStrategy";
import { captureGameEvents } from "@/games/jumper/testkit/events";
import { makeSpawners } from "@/games/jumper/testkit/spawn";
import type { PlatformSpawnSystem } from "@/games/jumper/systems/platformSpawnSystem";
import type { RocketBoosterSystem } from "@/games/jumper/systems/rocketBoosterSystem";
...
export async function createHarness(options: HarnessOptions = {}) {
	...
	const orthographicSize = viewportSize[1] / 2;
	const screen = new Screen(viewportSize, 1, orthographicSize);
	const world = new JumperWorld({ debug: options.debug ?? false });

	registerJumperComponents(world);

	const input = new FakeInputStrategy();
	const rnd = new Random(seed);
	const { physicsWorld } = registerJumperSystems(world, {
		screen,
		inputStrategy: input,
	...
	const spawn = makeSpawners(world, { screen });
