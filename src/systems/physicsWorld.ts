...
export class PhysicsWorld implements ISystem {
  ...
  constructor(world: World, viewportSize: Vec2) {
    this.#viewportSize = viewportSize;
    this.#world = world;
    this.#queryPadding = Math.max(viewportSize[0], viewportSize[1]) * 2;
  ...
  fixedUpdate(world: World) {
    ...
    if (cameraCenter) {
      Vec2.set(
        this.#tempQuerySize,
        this.#viewportSize[0] + this.#queryPadding * 2,
        this.#viewportSize[1] + this.#queryPadding * 2,
      );
    ...
    if (queryBounds) {
      ...
      const size = Math.max(this.#viewportSize[0], this.#viewportSize[1]) * 4;
      this.#quadTree = new Quadtree({
        x: -size / 2,
        y: -size / 2,
        width: size,
  ...
  sweptAABB(entity: Entity, moving: AABB, velocity: Vec2): SweepResult[] {
    ...
    if (!this.#tempEndAABB) {
      ...
      Vec2.set(this.#tempEndAABB.min, this.#tempEndMinCopy[0], this.#tempEndMinCopy[1]);
      Vec2.set(this.#tempEndAABB.max, this.#tempEndMaxCopy[0], this.#tempEndMaxCopy[1]);
    }
    ...
    if (!this.#tempSweepBounds) {
      ...
      Vec2.set(this.#tempSweepBounds.min, this.#tempUnionMin[0], this.#tempUnionMin[1]);
      Vec2.set(this.#tempSweepBounds.max, this.#tempUnionMax[0], this.#tempUnionMax[1]);
    }
  ...
  sweepAABBTest(
    ...
    if (velocity[0] === 0) {
      if (moving.max[0] <= candidate.min[0] || moving.min[0] >= candidate.max[0]) {
        return result;
      } else {
        entryTimeX = -Infinity;
        exitTimeX = Infinity;
    ...
    if (velocity[1] === 0) {
      if (moving.max[1] <= candidate.min[1] || moving.min[1] >= candidate.max[1]) {
        return result;
      } else {
        entryTimeY = -Infinity;
        exitTimeY = Infinity;
