...
it("saves actions before presenting the result and blocks stale input during pause", async () => {
	...
	draft.click();
	button('[data-ui-id="confirm-reward"]').click();
...
it("presents and saves a completed wave, then opens preparation and shop through the same screen", async () => {
	const { session, root, screen, button, onSave } = await setup();
	...
	button('[data-ui-id="continue-wave"]').click();
	expect(session.snapshot()).toMatchObject({
		phase: "prepare",
		pendingWorld: null,
	});
	expect(overlay.hidden).toBe(true);
	expect(root.querySelector(".defense-strategy-tabs")).not.toBeNull();
