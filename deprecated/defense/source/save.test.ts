...
function battle(game: DefenseSession) {
	...
	Reflect.deleteProperty(state, "triggers");
	return state;
}

it("reads fixed v2 saves and writes v7 without visual shot data", async () => {
	...
	expect(battle(game)).toMatchObject(migrated);
	...
	expect(game.snapshot().openSites).toBe(game.snapshot().sites.length);
	...
	expect(JSON.parse(game.save())).toEqual({ version: 7, state: battle(game) });
});
