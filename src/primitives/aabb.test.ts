...
describe("AABB", () => {
  ...
  describe("intersects", () => {
    it("should return true for intersecting AABBs", () => {
      const aabb1 = AABB.fromCenter(Vec2.fromValues(10, 10), Vec2.fromValues(4, 4));
      const aabb2 = AABB.fromCenter(Vec2.fromValues(11, 11), Vec2.fromValues(4, 4));
      expect(aabb1.intersects(aabb2)).toBe(true);
    });

    it("should return false for non-intersecting AABBs", () => {
      const aabb1 = AABB.fromCenter(Vec2.fromValues(0, 0), Vec2.fromValues(2, 2));
      const aabb2 = AABB.fromCenter(Vec2.fromValues(5, 5), Vec2.fromValues(2, 2));
