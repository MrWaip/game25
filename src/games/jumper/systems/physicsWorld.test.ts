...
import type { PhysicsWorld } from "@/games/jumper/systems/physicsWorld";
import { createHarness } from "@/games/jumper/testkit/harness";

describe("PhysicsWorld", () => {
	let world: JumperWorld;
	...
	describe("update", () => {
		it("заполняет quadtree сущностями", () => {
			...
			const testAABB1 = AABB.fromCenter(
				Vec2.fromValues(100, 100),
				Vec2.fromValues(50, 50),
			);
			const testAABB2 = AABB.fromCenter(
		...
		it("игнорирует отключённые коллайдеры", () => {
			...
			const testAABB = AABB.fromCenter(
				Vec2.fromValues(100, 100),
				Vec2.fromValues(50, 50),
			);
		...
		it("фильтрует сущности по viewport камеры, если камера есть", () => {
			...
			const testAABBInside = AABB.fromCenter(
	...
	describe("sweptAABB", () => {
		...
		it("находит коллизию при движении объекта в статичный объект", () => {
			...
			const movingAABB = AABB.fromCenter(
				Vec2.fromValues(100, 100),
				Vec2.fromValues(50, 50),
			);
			const velocity = Vec2.fromValues(150, 0);

			const results = physicsWorld.sweptAABB(
				movingEntity,
				movingAABB,
				velocity,
			);
		...
		it("игнорирует саму сущность при проверке коллизий", () => {
			...
			const entityAABB = AABB.fromCenter(
				Vec2.fromValues(100, 100),
				Vec2.fromValues(50, 50),
			);
	...
	describe("canPlaceAABB", () => {
		...
		it("возвращает false, если AABB пересекается с существующей сущностью", () => {
			const overlappingAABB = AABB.fromCenter(
				Vec2.fromValues(100, 100),
				Vec2.fromValues(50, 50),
			);
		...
		it("возвращает true, если AABB не пересекается", () => {
			const freeAABB = AABB.fromCenter(
	...
	describe("findFreeSpawnPosition", () => {
		...
		it("возвращает позицию, если базовая позиция свободна", () => {
			...
			const result = physicsWorld.findFreeSpawnPosition(
				undefined,
				basePos,
				size,
			);
