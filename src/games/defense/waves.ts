...
export function waveAt(number: number): Wave {
	...
	return {
		title: `${template.title} · этап ${cycle + 1}`,
		description: `${count} врагов с увеличенным здоровьем. ${template.description.replace("8 бродяг. ", "")}`,
...
export function waveDifficulty(wave: number) {
	...
	return {
		health: 1 + index * 0.18 + late * late * 0.045,
