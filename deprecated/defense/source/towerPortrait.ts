...
export function towerPortrait(kind: TowerKind): UiNode {
	return {
		...
		draw(ctx, rect) {
			...
			} else if (kind === "corrode") {
				box(-3, 1, 6, 10, color, 1);
				circle(0, 14, 10, color);
				circle(0, 12, 7, palette.towerBody);
				circle(2, 14, 3, color);
			} else if (kind === "amplifier") {
				circle(0, 12, 12, color);
				circle(0, 12, 9, palette.towerBody);
				circle(0, 12, 5, color);
				circle(0, 12, 2, palette.shatter);
				box(-14, 3, 4, 18, color, 1);
				box(10, 3, 4, 18, color, 1);
			} else {
				ctx.fillStyle = color;
				ctx.beginPath();
				ctx.moveTo(x, y);
