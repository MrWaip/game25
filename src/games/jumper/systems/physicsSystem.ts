...
export class PhysicsSystem implements JumperSystem {
					...
					new CollidedComponent(
						solids.map(({ entity, normal, time }) => {
							world.debug(() => {
								...
								const tranform = world.getComponent(
									entity,
									TransformComponent,
								)!;
								...
								world.debugAABB(
									AABB.fromCenter(this.#tempVec7, collider.size),
									"orange",
								);
							});
		...
		world.debug(() => {
			for (const {
				...
				Vec2.add(
					...
					triggerCollider.offset,
				);
				world.debugPersistentAABB(
					`trigger-${entity}`,
					AABB.fromCenter(this.#tempVec7, triggerCollider.size),
					"green",
				);
			}
		});
