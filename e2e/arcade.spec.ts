...
test("mobile construction, pause, save, reload and game switching", async ({
	...
	expect(await page.evaluate("window.__lifecycle().frames")).toBe(1);
	await page.getByRole("button", { name: "Платформер", exact: true }).tap();
...
test("winter build, full-screen reward and portal placement survive reload", async ({
	page,
}) => {
	...
	await expect(reward.getByRole("button")).toHaveCount(3);
	...
	await page.reload();
	await expect(reward).toBeVisible();
	await reward.getByRole("button", { name: "Разлом", exact: true }).tap();
	const entranceCell = (await buildableCells(page)).at(-1)!;
	await page
		.getByRole("button", { name: `Клетка ${entranceCell}`, exact: true })
		.tap();
	await page.getByRole("button", { name: "Здесь вход →", exact: true }).tap();
	const exitCell = (await buildableCells(page))[0];
	await page
		.getByRole("button", { name: `Клетка ${exitCell}`, exact: true })
		.tap();
	await page.getByRole("button", { name: "Связать портал", exact: true }).tap();
	await page.reload();
	await expect(
		page.getByRole("button", { name: "Начать волну 2", exact: true }),
	).toBeEnabled();
	...
	expect(saved.state.portal).toMatchObject({
		entrance: entranceCell - 1,
		exit: exitCell - 1,
	});
...
test("opening draft changes between codes and repeats after reload", async ({
	...
	for (const seed of ["opening-1", "opening-2", "opening-3", "opening-4"]) {
		...
		const choices = () =>
			page
				.locator(".defense-reward-screen button")
				.evaluateAll((buttons) =>
					buttons.map((button) => button.getAttribute("aria-label")),
				);
		await expect(page.locator(".defense-reward-screen button")).toHaveCount(3);
...
test("touch dragging a new tower from the canvas tray builds only on a valid drop", async ({
	...
	await page.getByRole("button", { name: `Клетка ${b}`, exact: true }).tap();
	const before = await read();
	await drag(await point(`[aria-label="Клетка ${a}"]`));
